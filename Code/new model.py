# -*- coding: utf-8 -*-
"""
Created on Sat Sep 28 13:16:56 2019
...
def forward(n, m, pi, A, b0,b, x):
    ...
    for t in range(1, n):
        for j in range(0, m):
            for k in range(0,m):
                alpha[t,j] = logSumExp(np.asarray(alpha[t-1, :])+np.asarray(A[:,j])+b[:,j,x[t]])
    return(alpha)
